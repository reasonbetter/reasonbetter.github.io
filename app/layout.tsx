...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" suppressHydrationWarning>
      <body id="top" className={`min-h-screen bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100 ${roboto.variable} ${roboto.className} font-light`}>
        <header className="sticky top-0 z-40 backdrop-blur bg-white/70 dark:bg-slate-950/70 border-b border-slate-200 dark:border-slate-800">
          <div className="mx-auto max-w-6xl px-4">
            <div className="flex h-14 items-center justify-between">
              ...
              <div className="flex flex-1 items-center justify-between md:justify-end gap-6">
                {/* Mobile left: Top link */}
                <Link href="/#top" className="md:hidden text-sm font-medium text-slate-700 dark:text-slate-300 hover:underline">Top</Link>

                {/* Mobile center nav */}
                <nav className="flex items-center justify-center gap-6 text-base font-semibold text-slate-700 dark:text-slate-300 md:hidden">
